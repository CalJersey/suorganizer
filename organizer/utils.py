...
class PageLinksMixin:
    ...
    def previous_page(self, page):
        if (page.number > 2 and page.has_previous()):
...
''' adding startup field to form data from url parameter after it is submitted
(not recommended)
    ...
    def get_form_kwargs(self):
        ...
        if self.request.method in ('POST', 'PUT'):
            self.startup = get_object_or_404(
                Startup,
                slug__iexact=self.kwargs.get(self.startup_slug_url_kwarg)
            )
...
'''  no longer used, but included for reference
    ...
    def get_object(self):
        ...
        if slug is None:
            raise AttributeError(
                "{c} expects {p} parameter from URL pattern.".format(
        ...
        if self.model:
            ...
            raise ImproperlyConfigured(
                "{c} needs {a} attribute specified to work.".format(
                    c=self.__class.__name__,
                    a='model'))
